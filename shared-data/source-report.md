...
No 50-product snapshot was created. Creating synthetic records and claiming they were collected from Supersports would be inaccurate. Use `manual-products-template.csv` with user-supplied public product facts, then run `npm run convert:manual-data`, `npm run validate:data`, `npm run sync:data`, and `npm run verify:data`.
