...
export function assertSnapshot(products) {
  ...
  for (const product of products) {
    ...
    if (product.sourceUrl !== null && urls.has(product.sourceUrl)) errors.push(`Duplicate sourceUrl: ${product.sourceUrl}`)
    if (product.sourceUrl !== null) urls.add(product.sourceUrl)
    ...
    if (product.sourceUrl !== null && !/^https:\/\//.test(product.sourceUrl)) errors.push(`Invalid source URL: ${product.slug}`)
    if (product.sourceImageUrl !== null && !/^https:\/\//.test(product.sourceImageUrl)) errors.push(`Invalid source image URL: ${product.slug}`)
    ...
    if (!product.image.startsWith('/products/') || !existsSync(resolve(dataDir, 'images', basename(product.image)))) errors.push(`Missing local image: ${product.image}`)
  }
