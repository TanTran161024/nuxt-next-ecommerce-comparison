...
const resultRoot = resolve(experimentRoot, pilotDirectory ?? '.')
const input = resolve(resultRoot, 'raw-data/build-results.csv')
const output = resolve(resultRoot, 'summary/build-summary.csv')
if (!existsSync(input)) {
  ...
  if (lines.length < 2) {
    ...
    for (const [framework, group] of groups) {
      ...
      const variance = values.length > 1 ? values.reduce((sum, value) => sum + (value - mean) ** 2, 0) / (values.length - 1) : ''
      ...
      summary.push([framework, group.length, values.length, group.length - values.length, mean, values.length > 1 ? Math.sqrt(variance) : '', median, values.length ? ordered[0] : '', values.length ? ordered.at(-1) : ''])
    }
    const csv = summary.map((row) => row.join(',')).join('\n').concat('\n')
    mkdirSync(resolve(resultRoot, 'summary'), { recursive: true })
    writeFileSync(output, csv)
    writeFileSync(resolve(resultRoot, 'summary/build-summary.md'), ['# Build summary', '', `Source: ${input}`, '', 'Không đủ số lần đo để tính độ lệch chuẩn mẫu khi một framework chỉ có một build thành công.', 'Failed runs remain counted separately and are not converted to zero seconds.', '', '```csv', csv.trimEnd(), '```', ''].join('\n'))
