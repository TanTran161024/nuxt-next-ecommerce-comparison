...
const testLogPath = resolve(outputRoot, 'test-results', `functional-${now().replaceAll(':', '-')}.log`)
...
if (nuxt.exitCode !== 0 || next.exitCode !== 0) {
  ...
  const csvPath = resolve(outputRoot, 'test-results/functional-test-cases.csv')
  const lines = readFileSync(csvPath, 'utf8').split(/\r?\n/)
  const automaticallyCovered = new Set(['FT-01', 'FT-02', 'FT-03', 'FT-04', 'FT-05', 'FT-06', 'FT-07', 'FT-08', 'FT-09', 'FT-10'])
  const manualReview = new Set(['FT-11', 'FT-12', 'FT-13', 'FT-14'])
  const updated = lines.map((line, index) => {
    ...
    if (manualReview.has(cells[0]) && !cells[5]) cells[5] = 'NEEDS_MANUAL_REVIEW'
