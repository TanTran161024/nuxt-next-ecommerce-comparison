...
const records = rows.filter(Boolean).map((row, index) => {
  ...
  const slug = input.sourceUrl.split('/').filter(Boolean).at(-1)
  ...
  return {
    ...
    image: `/images/${slug}.svg`, sourceImageUrl: input.sourceImageUrl || null, sourceUrl: input.sourceUrl, featured: index < 8, capturedAt,
  }
})
mkdirSync(resolve(dataDir, 'images'), { recursive: true })
for (const product of records) {
  const label = `${product.brand} · ${product.category} · ${product.color}`.replace(/[&<>]/g, '')
  writeFileSync(resolve(dataDir, product.image.slice(1)), `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" role="img" aria-label="${label}"><rect width="800" height="600" fill="#e2e8f0"/><text x="400" y="300" text-anchor="middle" font-family="Arial" font-size="28" fill="#0f172a">${label}</text></svg>`)
}
